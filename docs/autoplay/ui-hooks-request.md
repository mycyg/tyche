...
| 5 | `data-ending-id`、`data-ending-category` | `.ending-page` | 用正则从 `.ending-intro .eyebrow` 的 `X31 / 类别 · …` 里取编号 | 眉题格式一变就取不到结局编号，只能回落到读 `localStorage` 的存档 |
...
| 10 | `data-day`、`data-ap` | `.rpg-location`、`.rpg-ap` | 从「第 01 天」「行动 5」的文字里取数字 | 数字格式或单位一改，日志里的天数与行动值就会记错 |

## 补充说明

- 任务说明里提到的 `?preview-ending=` 查询参数在当前 `main`（`de4d802`）的 `src/**` 里搜不到任何实现，驱动器没有依赖它。若要为结局页做定向回归，需要先补这个入口。
