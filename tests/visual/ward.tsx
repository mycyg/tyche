...
function fixture(profile: number, mobile: boolean, bed: number) {
  ...
  r.world={x:132,y:148,facing:0,day:1};
  return r;
}
function Review() {
  ...
  const reset=(p=profile,m=mobile,b=bed)=>{setProfile(p);setMobile(m);setBed(b);setRun(fixture(p,m,b));setClose(false);setRevision(x=>x+1);};
  return <><nav style="height:52px;display:flex;gap:12px;align-items:center;padding:4px 12px;position:relative;z-index:20;background:#152138">
    ...
    <WorldStage key={revision} r={r} motion={motion} frozen={close} onEncounter={()=>{}} onPatient={()=>setClose(true)} onAmbient={()=>{}} onAction={()=>{}} onMenu={()=>{}} onPosition={()=>{}} onTitle={()=>{}}>
