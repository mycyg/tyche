...
export type WalkAtlasId = 'staff-walk' | 'ward-life-walk' | 'patient-walk' | 'family-walk' | 'conflict-walk';
export type ActionAtlasId = 'ward-actions' | 'family-actions' | 'conflict-actions';
...
export function walkFrame(id: WalkAtlasId, row: number, facing: number, step: number, x: number, y: number): FrameRect {
  ...
  return frame(atlas, clampRow(atlas, row), WALK_COLUMN[((facing % 4) + 4) % 4] + (step & 1), x, y);
}
